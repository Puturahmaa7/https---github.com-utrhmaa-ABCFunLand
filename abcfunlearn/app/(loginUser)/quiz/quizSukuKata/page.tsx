...
export default function QuizSukuKataPage() {
  ...
  const playSoal = () => {
    const audio = new Audio(
      `/suaraSukuKata/${questions[currentIndex].correct.toLowerCase()}.mp3`
    );
    audio.play();
  };

  /* 🔊 OPTION SOUND */
  const playOption = (suku: string) => {
    const audio = new Audio(`/suaraSukuKata/${suku.toLowerCase()}.mp3`);
    audio.play();
  };
  ...
  const generateQuiz = () => {
    ...
    for (let i = 0; i < TOTAL_SOAL; i++) {
      const correct =
        sukuKataList[Math.floor(Math.random() * sukuKataList.length)];

      const wrong = sukuKataList
        .filter((s) => s !== correct)
  ...
  return (
    <div style={container}>
      ...
      <div style={instruksi}>Dengarkan lalu pilih suku kata</div>
      ...
      <div style={grid}>
        {current.options.map((s) => {
          const benar = s === current.correct;
          const dipilih = s === jawaban;
          ...
          return (
            ...
            >
              ...
              >
                <Image
                  src={`/suku_kata/${s.toLowerCase()}.png`}
                  alt={`Suku kata ${s}`}
                  width={90}
                  height={90}
                  style={{ marginBottom: "8px" }}
                />
                <div style={opsiTextBig}>{s}</div>
              </div>
            </button>
          );
        })}
